...
export function Header() {
    ...
    return(
         ...
         <MotionTransition>
            <nav className="flex flex-wrap items-center justify-between max-w-5xl p-4 mx-auto md-py-8 cursor-pointer">
                    ...
                    <div className={`${openMobileMenu ? 'block' : 'hidden'} w-full md:block md:w-auto`}>
                        <div className="flex flex-col p-4 mt-4 md:p-0 md:flex-row md:space-x-8 md:mt-0 md:border-0">
                           {dataHeader.map(({id, name, idLink}) => (
                            <div key={id} className="text-[20px] px-4 py-4 transition-all duration-700 ease-in-out hover:text-secondary">
                                <Link href={idLink}  className="text-lg " />
                                  {name}
                            </div>
                           ))} 
                        </div>
